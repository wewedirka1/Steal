...
-- Новая функция RandomMoveWhenNear
local isRandomMoveActive = false

Tabs.Main:AddToggle("RandomMoveToggle", {
    Title = "Random Move When Near",
    Description = "Moves randomly when another player is nearby",
    Default = false,
    Callback = function(state)
        isRandomMoveActive = state
        if state then
            task.spawn(function()
                while isRandomMoveActive do
                    local character = player.Character
                    if character and character:FindFirstChild("HumanoidRootPart") then
                        local humanoidRootPart = character.HumanoidRootPart
                        local nearbyPlayer = false
                        
                        -- Проверка на наличие игроков поблизости
                        for _, otherPlayer in ipairs(game.Players:GetPlayers()) do
                            if otherPlayer ~= player and otherPlayer.Character and otherPlayer.Character:FindFirstChild("HumanoidRootPart") then
                                local distance = (humanoidRootPart.Position - otherPlayer.Character.HumanoidRootPart.Position).Magnitude
                                if distance < 20 then -- Расстояние в 20 studs
                                    nearbyPlayer = true
                                    break
                                end
                            end
                        ...
                        if nearbyPlayer then
                            ...
                            local randomDistance = math.random(5, 15) -- Случайное расстояние от 5 до 15
                    ...
                    task.wait(0.5) -- Задержка между проверками
                end
            end)
        end
    end
...
task.spawn(function()
    while true do
        ...
        Timer:SetTitle(string.format("Time: %02d:%02d:%02d", math.floor(et / 3600), math.floor((et % 3600) / 60), эт % 60))
        task.wait(1)
    end
end)
